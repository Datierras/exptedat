// /js/app.js — versión completa usando firebase-compat.js (bindings explícitos)

// ====== Bindings explícitos desde firebase-compat.js ======
if (!window.firebase) {
  console.error("firebase no está en window. Verificá orden de scripts en index.html.");
}
const auth = window.auth;
const db = window.db;
const Timestamp = window.Timestamp;

console.log("Iniciando app.js | auth:", !!auth, "| db:", !!db, "| Timestamp:", !!Timestamp);
...
function initApp() {
  ...
  if (!auth || !db || !Timestamp) {
    console.error("Faltan bindings (auth/db/Timestamp). Asegurate de cargar firebase-compat.js ANTES de app.js.");
    if (authError) authError.textContent = 'Error de inicialización. Revisá consola.';
    return;
  }
...
function handleAuthError(error, outEl){
  ...
  if (outEl) outEl.textContent = map[error && error.code] || ('Ocurrió un error inesperado. ' + (error?.message || ''));
}
...
function setupExtractoAutofill() {
  ...
  async function runQuery(numeroVal, exact, letra, anio) {
  ...
  async function fetchLastExtracto() {
    ...
    const nAsNum = Number(numeroRaw);
    const candidates = isNaN(nAsNum) ? [numeroRaw] : [nAsNum, String(numeroRaw)];

    if (letra && anio) {
      for (const n of candidates) {
        const d = await runQuery(n, true, letra, anio);
    ...
    for (const n of candidates) {
      const d = await runQuery(n, false);
      if (d?.extracto && !extractoInp.dataset.userEdited) {
        extractoInp.value = d.extracto;
        extractoInp.dataset.autofilled = '1';
        lockExtracto(true);
...
function setupEnvioGrupal(){
  ...
  if (envio.btnConfirmar) envio.btnConfirmar.addEventListener('click', async ()=>{
    ...
    siNoHay: if (!envio.lista.length) { alert('No hay expedientes cargados.'); break siNoHay; }
